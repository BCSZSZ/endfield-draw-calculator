...
- **角色概率层**：6 星概率规则与干员页一致（0.8% 基础、66 后线性抬升、80 抽必 6 星）；6 星中 UP 占 50%；5 星基础 8% 且 10 抽内保底。
- **配额折算**：角色抽卡中 6 星/5 星/4 星分别累积武器配额 `+100/+10/+1` 单位；再按 `quota/1980` 的规则折算为武器 10 连次数。
- **联合达成**：对每个终态质量，先判断角色目标是否达成，再乘以“对应武器 10 连次数下达成武器目标”的累计概率，最后加总得到综合概率。
- **额外规则与前提**：当前页面固定初始水位为 0，启用 30/120/240 的历史赠送规则，使用阈值裁剪加速；优先读取预计算结果，未命中时实时精确计算。
"""
    )
